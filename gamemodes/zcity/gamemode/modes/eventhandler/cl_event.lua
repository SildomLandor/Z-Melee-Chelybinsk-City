...
function MODE:HUDPaint()
    ...
    draw.SimpleText("Челябинск | "..eventname, "ZCity_Veteran_big", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    ...
    draw.SimpleText("Ты - "..Rolename , "ZCity_Veteran_big", sw * 0.5, sh * 0.5, ColorRole, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
net.Receive("event_end", function()
	...
	wonply = IsValid(ent) and ent or nil
	...
	zb.EndMenu.Open({
		...
		rowStyle = function(ply)
			if ply.won then return { winner = true } end
