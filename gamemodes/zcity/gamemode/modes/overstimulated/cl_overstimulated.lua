...
local roles = {
	[0] = {
		objective = "Обезвредте обдолбанного стрелка.",
		name = "SWAT",
		color1 = Color(68, 10, 255),
		color2 = Color(68, 10, 255)
	},
	[1] = {
		objective = "Обдолбанный стрелок рядом. Прячься. Ищи оружие. Беги.",
		name = "Жертва",
		color1 = Color(255, 255, 255),
		color2 = Color(255, 255, 255)
	},
	[2] = {
		objective = "Опять прячутся... Ничего... Я чую страх.",
		name = "Передозированный",
...
function MODE:HUDPaint()
	...
	local lastStandEndAt = GetGlobalFloat("overstimulated_laststand_end_at", 1)
	...
	if ply:Team() == 2 then
		...
		if (waiting or fadeout > 0) and now >= revealEnd then
			...
			draw.SimpleText("Ты " .. roleName, "ZC_MM_Title", sw * 0.5, sh * 0.45, red, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			draw.SimpleText("Я близко... как раз вовремя...", "ZCity_Veteran", sw * 0.5, sh * 0.54, white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			draw.SimpleText("До прихода: " .. string.FormattedTime(math.max(shooterSpawnAt - now, 0), "%02i:%02i"), "ZCity_Veteran", sw * 0.5, sh * 0.62, white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	end
	...
	if zb.ROUND_START + 8.5 > now then
		...
		draw.SimpleText("Передозированный", "ZC_MM_Title", titleX, titleY, Color(220, 35, 35, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		draw.SimpleText("Ты " .. roleName, "ZC_MM_Title", sw * 0.5, sh * 0.5, roleColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	if reportData and reportUntil > now then
		...
		draw.SimpleText(reportData.title or "Отчет об инциденте", "ZC_MM_Title", sw * 0.5, sh * 0.25, Color(200, 50, 50, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText(reportData.winner or "Исход: неизвестен", "ZCity_Veteran", sw * 0.5, sh * 0.34, Color(255, 255, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		local y = sh * 0.42
		draw.SimpleText("Пострадавшие:", reportFont, sw * 0.5, y, Color(220, 220, 220, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		y = y + lineHeight * 0.65
		draw.SimpleText("Выжившие:", reportFont, sw * 0.5, y, Color(180, 180, 180, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		draw.SimpleText("Выстрелы стихли. Остался только звон в ушах и запах пороха, впитавшийся в стены.", reportFont, sw * 0.5, math.min(y + lineHeight * 1.25, sh * 0.9), Color(190, 190, 190, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end
