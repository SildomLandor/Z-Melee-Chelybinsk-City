...
concommand.Add("tdm_setrounds", function(ply, cmd, args)
    ...
    PrintMessage(HUD_PRINTTALK, "Командный бой установлен на "..MODE.Rounds..". Раундов осталось: "..zb.RoundsLeft)
end)
...
COMMANDS.nextcsround = {
	function(ply, args)
		...
		if string.lower(args[1]) == "bomb" then
            zb.nextcsround = "bomb"
            PrintMessage(HUD_PRINTTALK, "Выбранный раунд - Бомба")
        end

        if string.lower(args[1]) == "hostage" then
            zb.nextcsround = "hostage"
            PrintMessage(HUD_PRINTTALK, "Выбранный раунд - Заложники")
        end
	end,
	0
}
...
function MODE:EndRound()
    ...
    elseif zb.rtype == "hostage" then
        ...
        if IsValid(zb.hostage) and not zb.hostage.organism.alive then
            ...
            if zb.HarmDoneDetailed[zb.hostage:EntIndex()] then
                ...
                PrintMessage(HUD_PRINTTALK, (maxTeam == 0 and "Террористы" or "Контр-террористы") .. " убили заложника")
            else
                winner = 3
            end
        end
    ...
    local winnerprt = (winner == 1 and "Контр-террористы") or (winner == 0 and "Террористы") or "Никто"
    
    PrintMessage(HUD_PRINTTALK, winnerprt.." выиграл раунд.")
	...
	if winsTeam0 > winsTeam1 then
		for _, ply in ipairs(team.GetPlayers(1)) do
			...
			ply:ChatPrint("Вы получили компенсацию в 1000 денег, потому что ваша команда проигрывает.")
		end
    ...
    if zb.RoundsLeft > 0 then
        ...
        if winner then
            ...
            PrintMessage(HUD_PRINTTALK, winnerprt.." выиграл игру.")
        end
