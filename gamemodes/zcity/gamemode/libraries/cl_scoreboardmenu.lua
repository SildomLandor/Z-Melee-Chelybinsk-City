-- cl_scoreboardmenu.lua
-- Вся логика скорборда, мутов и связанных с ними функций
...
hook.Add("player_connect", "playerconn", function(data)
...
hook.Add("InitPostEntity", "zcity_muted", function()
    if file.Exists("zcity_muted.txt", "DATA") then
        local json = file.Read("zcity_muted.txt", "DATA")
        if json then
            hg.playerInfo = util.JSONToTable(json)
