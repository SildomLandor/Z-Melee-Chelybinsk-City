...
SWEP.Instructions = "Боевой нож военного образца, предназначенный для нейтрализации противника в ходе боевых действий и специальных операций."
SWEP.Category = "Weapons - Melee"
SWEP.Instructions = "Это ваш верный нож с фиксированным лезвием из углеродистой стали.\\n\\nЛКМ для атаки.\\nR + ЛКМ, чтобы изменить режим атаки.\\nПКМ, чтобы заблокировать."
SWEP.Spawnable = true
...
function SWEP:CustomThink()
    ...
    if self.SuicideRequest then
        self.SuicideRequest = nil
        local owner = self:GetOwner()
        if self.CanSuicide and owner and owner.suiciding and not self.SuicideStart then
            self.SuicideStart = CurTime()
            if SERVER then
                local dmgInfo = DamageInfo()
                dmgInfo:SetDamageType(DMG_SLASH)
