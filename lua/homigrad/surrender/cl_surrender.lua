...
hook.Add("radialOptions", "surrender_option", function()
    ...
    if inKneel then
        hg.radialOptions[#hg.radialOptions + 1] = {
            ...
            inHandsBehindHead and "На коленях + Руки за голову\nПКМ - Опции" or "На коленях + Руки вверх\nПКМ - Опции"
        }
        return
    end
        ...
        hg.radialOptions[#hg.radialOptions + 1] = {
            function(mouseClick)
                if mouseClick == 2 then
                    if inHandsBehindHead then
                        hg.CreateRadialMenu({
                            { function() ExitSurrender(ply) end,       "Прекратить сдачу"          },
                            { function() EnterKneel(ply) end,          "Опуститься на колени"      },
                            { function() ExitHandsBehindHead(ply) end, "Руки вверх (обычно)"       },
                        })
                    else
                        hg.CreateRadialMenu({
            ...
            inHandsBehindHead and "Руки за голову\nПКМ - Опции" or "Сдаюсь\nПКМ - Опции"
        }
    else
        local wep = ply:GetActiveWeapon()
