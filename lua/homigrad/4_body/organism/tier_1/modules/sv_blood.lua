...
module[2] = function(owner, org, mulTime)
	...
	if org.vomitInThroat then
		...
		if mat and mat:GetAngles():Right()[3] < 0.25 then
			...
			hg.orgSquirtSend(ent, bon, mat, mat:GetTranslation() + mat:GetAngles():Right() * 6 + mat:GetAngles():Forward() * 1, mat:GetAngles():Right() * 2 * math.Clamp(org.pulse / 70, 0.4, 1))
			net.Broadcast()
	...
	if org.coma or (org.otrub and ((not bleeding_will_stop and not (canwakeup_pain and org.blood > 3000)) or (org.brain > 0.4) or (org.pulse < 15) or (org.o2[1] < 5) or (org.trachea >= 0.5) or org.heartstop or (org.spine3 >= hg.organism.fake_spine3) or (org.spine2 >= hg.organism.fake_spine2))) then
